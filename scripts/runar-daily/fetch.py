...
USER_AGENT = "abstinensa-runar-daily/1.0 (+https://abstinensa.no)"
...
def fetch_all(sources_path: Path) -> list[dict]:
    ...
    for region_key, region_cfg in (config.get("regions") or {}).items():
        max_age_hours = region_cfg.get("max_age_hours", DEFAULT_MAX_AGE_HOURS)
        cutoff = now - timedelta(hours=max_age_hours)
        print(f"Region: {region_key} (siste {max_age_hours}t)")
